...
if ~exist('./../Result/example-3','dir')
    mkdir ./../Result/example-3
end
...
for curstep=1:totalsteps
  %%   pause
    time(curstep+1)=time(curstep)+dt;
     dislength=0;
        for mm=1:length(links)
            dislength=dislength+norm(rn(links(1),1:3)-rn(links(2),1:3));
        end
        density(curstep)=dislength/(L*L*L);
        
        
    appliedstress=appliedstress+appliedstressrate*dt; % changed!cyn
    ...
    [ep_inc1,wp_inc]=calcplasticstrainincrement(rnnew,rn,links,(2*plim)^3);
...
for idx = 1:size(im,2)
    ...
    if idx == 1
        imwrite(A,map,['./../Result/example-3/conf.gif'],'gif','LoopCount',Inf,'DelayTime',0.1);
    else
        imwrite(A,map,['./../Result/example-3/conf.gif'],'gif','WriteMode','append','DelayTime',0.1);
    end
end
figure(2)
clf
hold on;
plot([0 appliedstress(3,3)/E+ep_inc1(3,3)],[0 appliedstress(3,3)])% changed!cyn
plot([0 appliedstress(3,3)/E],[0 appliedstress(3,3)],'r--')% changed!cyn
xlabel('strain');% changed!cyn
ylabel('stress');% changed!cyn
print(['./../Result/example-3/stress-strain-',num2str(curstep)],'-djpeg','-r300');


figure(3)
clf
hold on;
plot(time(1:curstep),density(1:curstep));
xlabel('Time');% changed!cyn
ylabel('Density');% changed!cyn
print(['./../Result/example-3/dis_density',num2str(curstep)],'-djpeg','-r300');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%