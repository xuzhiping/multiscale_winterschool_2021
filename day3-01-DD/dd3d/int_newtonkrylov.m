function [rnnew,vn,dt,fn,fseg]=int_newtonkrylov(rn,dt,dt0,MU,NU,a,Ec,links,connectivity,appliedstress,rmax,rntol,mobility)
...
while ierr~=0
    dt=0.5*dt;
    [sol, it_hist, ierr, x_hist] = nkiter(rnvec,'calposerr',tol, parms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
...
function [sol, it_hist, ierr, x_hist] = nkiter(x,f,tol,parms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility)
...
%
f0 = feval(f,x,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
fnrm = norm(f0);
...
%
    ...
    [step, errstep, inner_it_count,inner_f_evals] = ...
         dkrylov(f0, f, x, gmparms, lmeth,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
...
%
    ...
    xt = x + lambda*step;
    ft = feval(f,xt,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
...
function [step, errstep, total_iters, f_evals] = ...
    dkrylov(f0, f, x, params, lmeth,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility)
...
% for this purpose
%
    [step, errstep, total_iters] = dgmres(f0, f, x, gmparms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
    kinn = 0;
%
...
%
    while total_iters == lmaxit & ...
          ...
          kinn < restart_limit
        kinn = kinn+1;
        [step, errstep, total_iters] = dgmres(f0, f, x, gmparms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,step);
    end
...
%
%
function z = dirder(x,w,f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility)
...
%
del = x+epsnew*w;
f1 = feval(f,del,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
z = (f1 - f0)/epsnew;
%
%
function [x, error, total_iters] = dgmres(f0, f, xc, params, rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,xinit)
...
%
    v(:,k+1) = dirder(xc, v(:,k), f, f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
    normav = norm(v(:,k+1));
%
...
%
%
function [pos_error]=calposerr(rnvec1,rnvec0,vnvec0,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);

    [vnvec,fn,fseg]=drndt(dt,rnvec1,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
